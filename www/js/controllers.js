...
.controller('ChatDetailCtrl', function($scope, $stateParams, Chats, $ionicLoading, $ionicPopover) {
  ...
  $scope.showType2 = function(chat){
   ...
   var type_template_2 = '<ion-popover-view>' + '<ion-header-bar>' +
      ...
      '</ion-content>' + '</ion-popover-view>';
   var nature_template = 
      '<ion-popover-view>' + '<ion-header-bar>' +
      '<label class="item item-input animated fadeInUp">'+
      '<i class="icon ion-search placeholder-icon"></i>'+
      '<input type="search" placeholder="Search Items" ng-model="searchText">'+
    '</label>'+
      '</ion-header-bar>'+ '<ion-content>' +
      ...
      '<div class="card animated fadeInUp">'+
          '<table id="table_id">'+
            ...
            '<th> Increases</th>'+
            '<th> Decreases</th>'+
            '<tr ng-repeat="x in natures | filter:searchText">'+
              '<td>{{ x.name }}</td>'+
              '<td class="text-positive">{{ x.increases }}</td>'+
              '<td class="text-negative">{{ x.decreases }}</td>'+
            '</tr>'+
          '</table>'+
        '</div>'+
        '</ion-content>' + '</ion-popover-view>';
   ...
   $scope.nature_popover = $ionicPopover.fromTemplate(nature_template, {
      scope: $scope,
      animation: $scope.animation
   });
   $scope.egg_groups_popover = $ionicPopover.fromTemplate('templates/popovers/egg_groups.html', {
      scope: $scope,
      animation: $scope.animation
   });
